...
//-------------INTERACCIONES:
//Funciona con los clicks derecho e izquierdo, movimiento de mouse en x,  con el mouse sobre el torso y  teclas "a/A" (se libera el personaje) y "s/S" (vuelve a la normalidad).--------------------------------------------------------------------------------------
...
function draw() {
  ...
  //----TORSO
  var btnTorsoX = 352;
  var btnTorsoY = 162.56;
  var btnTorsoW = 95.4;
  var btnTorsoH = 125.28;

  rect(btnTorsoX, btnTorsoY, btnTorsoW, btnTorsoH);
  ...
  if (
    //posición del mouse
    mouseX > btnTorsoX &&
    mouseX < btnTorsoX + btnTorsoW &&
    mouseY > btnTorsoY &&
    mouseY < btnTorsoY + btnH
  ) {
    //movimiento de la jaula
    if (contador % 3) {
      rotate(contador * 0.004);
    } else if (contador % 4) {
      rotate(contador * -0.004);
    }
    ...
    line(568.92, 120.8, 640.88, 55.28);
  } //fin if movimiento jaula

  pop();
  //fin interacción torso
  ...
  /*Animación del personaje y la jaula, si se presiona la "a" gira y se dibuja cambiando la escala de acuerdo a la posición de mouseX, si luego de presionar la a se clickea, el color cambia y la pantalla se limpia. Al presionar "s" finaliza esa animación y vuelve a aparecer el personaje en el centro.*/
...
if (contador >60)
  {angulo= angulo -(contador/100)}
  else {
    angulo = angulo + contador / 25;
  }
  contador++;
  if (contador > 100) {
    contador = 0;
